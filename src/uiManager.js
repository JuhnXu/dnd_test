...
export class UIManager {
  ...
  render({ units, currentUnit, mode, battleEnded }) {
  ...
  renderStatus(currentUnit, mode) {
    ...
    this.statusEl.innerHTML = `
      ...
      <strong>移动：</strong>${currentUnit.move}　<strong>攻击：</strong>+${currentUnit.attackBonus}，${currentUnit.damageDice}<br>
  ...
  renderButtons(currentUnit, battleEnded) {
    ...
    this.moveBtn.disabled = !isPlayerTurn || currentUnit.hasMoved;
